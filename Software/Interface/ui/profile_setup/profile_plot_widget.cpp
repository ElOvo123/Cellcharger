...
void ProfilePlotWidget::setActiveStepMarkerValues(bool visible, double time, double voltage, double current,
                                                  double temperature)
{
    m_activeStepVisible = visible;
    m_activeStepTime = time;
    ...
    m_activeStepValue =
        ProfilePlotLogic::valueForDisplay({time, voltage, current, temperature, "Linear", 1.0}, m_displayMode);
    updatePlot();
}
...
void ProfilePlotWidget::wheelEvent(QWheelEvent* event)
{
    ...
    const std::optional<ProfilePlotViewState> zoomed =
        ProfilePlotLogic::zoomedViewState(plotRect, event->position(), event->angleDelta().y(),
...
void ProfilePlotWidget::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    else if (m_selecting && event->button() == Qt::LeftButton)
    {
        ...
        const std::optional<ProfilePlotViewState> selected = ProfilePlotLogic::selectedViewState(
            plotRect, m_selectionRect, {m_viewMinTime, m_viewMaxTime, m_viewMinValue, m_viewMaxValue});
...
void ProfilePlotWidget::paintEvent(QPaintEvent* event)
{
    ...
    auto drawCurve = [&](ProfilePlotWidget::DisplayMode mode, const QColor& color)
    {
        ...
        for (size_t i = 0; i < plotPoints.size() - 1; ++i)
        {
            ...
            for (int step = 0; step <= 50; ++step)
            {
                ...
                double y =
                    plotRect.bottom() - (value - viewMinValue) / (viewMaxValue - viewMinValue) * plotRect.height();
    ...
    else
    {
        ...
        if (m_displayMode == DisplayMode::Current)
            color = Qt::green;
    ...
    for (const auto& sp : plotPoints)
    {
        ...
        double y = plotRect.bottom() - (ProfilePlotLogic::valueForDisplay(sp, m_displayMode) - viewMinValue) /
                                           (viewMaxValue - viewMinValue) * plotRect.height();
    ...
    {
        const double x =
            plotRect.left() + (m_activeStepTime - viewMinTime) / (viewMaxTime - viewMinTime) * plotRect.width();
    ...
    if (m_activeStepVisible)
    {
        ...
        {
            const Setpoint markerPoint{
                m_activeStepTime, m_activeStepVoltage, m_activeStepCurrent, m_activeStepTemperature, "Linear", 1.0};
    ...
    QRect legendRect(plotRect.right() - legendWidth - legendMargin, plotRect.top() + legendMargin, legendWidth,
                     legendHeight);
    painter.setPen(QPen(Qt::black, 1));
...
double ProfilePlotWidget::interpolate(double t0, double v0, double t1, double v1, double t, const QString& curveType,
                                      double rampStep)
{
    if (t1 == t0)
        return v0;
